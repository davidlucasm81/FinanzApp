...
| Suscripciones / pagos in-app | Google Play Billing Library + RevenueCat | RevenueCat gestiona la suscripción Premium y verifica los recibos con Google sin necesidad de backend propio ni de vincular el plan Blaze; su nivel gratuito no requiere tarjeta de crédito. Ver Fase 16 y la sección de "Monetización". |
| Verificación Premium | RevenueCat + Whitelist + RSA Local | Triple validación: RevenueCat (servidor), Whitelist en Firestore (para desarrolladores/familia) y verificación de firma RSA local (seguridad offline). |
...
- **Seguridad Firestore**: Las reglas de Firestore impiden que un usuario normal modifique su campo `isPremium` a `true` si no existe una firma válida de Google o si no está en la `premium_whitelist`.
...
- **Suscripción Premium**: Gestionada por **RevenueCat**. RevenueCat verifica los recibos de compra con los servidores de Google de forma gratuita (plan de 0$ hasta 2.5k$/mes) y sin necesidad de tarjeta de crédito para el desarrollador.
...
- **2026-07-29**: **Nueva Fase 16 — Monetización con anuncios y suscripción Premium**. Petición explícita del propietario: los anuncios no deben bloquear el uso de la app, sino mostrarse como una caja/banner en puntos concretos. Se utiliza **RevenueCat** para la gestión de suscripciones (gratis y sin tarjeta) y se añade una **Whitelist** en Firestore para acceso gratuito de familiares/desarrolladores. La seguridad se refuerza con **Reglas de Firestore** (que validan el email contra la whitelist) y **verificación RSA local** de firmas de Google Play. Se elimina la limitación de "no verificación en servidor" al usar RevenueCat.
