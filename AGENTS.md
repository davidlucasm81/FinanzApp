...
| Suscripciones / pagos in-app | Google Play Billing Library + RevenueCat | RevenueCat gestiona el Premium de por vida (pago único) y verifica los recibos con Google sin necesidad de backend propio ni de vincular el plan Blaze; su nivel gratuito no requiere tarjeta de crédito. Ver Fase 16 y la sección de "Monetización". |
| Verificación Premium | RevenueCat + Whitelist + RSA Local | Triple validación: RevenueCat (servidor), Whitelist en Firestore (para desarrolladores/familia) y verificación de firma RSA local (seguridad offline). Indicadores visuales específicos en la UX (Estrella Verde para Whitelist). |
...
├── data/
│   ├── model/                    // POJOs: User, Family, Member, Invitation, Account, Transaction, Category, ImportRowResult, FamilyMembership, FcmToken, Notification, Settlement (Fase 18)
│   ├── repository/               // AuthRepository, FamilyRepository, AccountRepository, TransactionRepository, CategoryRepository, NotificationRepository, NotificationSettingsRepository (Fase 8 bis), SettlementRepository (Fase 18)
│   ├── sharedexpenses/            // (Fase 18) BalanceCalculator: calcula saldos netos por miembro y la lista simplificada de pagos sugeridos a partir de transactions + settlements ya leídos (lógica pura, sin dependencias de Firestore, fácil de testear)
...
│   ├── sharedexpenses/            // (Fase 18) BalancesFragment (saldos netos + pagos sugeridos + botón "Marcar como saldado"), SettlementHistoryFragment
...
families/{familyId}
  ...
  createdBy: uid
  createdAt: timestamp
  mode: "normal" | "shared_expenses" // (Fase 18) modo de la unidad familiar. "normal" es el comportamiento de siempre (posición neta conjunta). "shared_expenses" activa además el reparto de gastos y el cálculo de saldos entre miembros. Ausente en familias creadas antes de la Fase 18 → tratar como "normal" (self-heal, mismo patrón que el resto del documento; no requiere migración porque el valor por defecto ya es el comportamiento actual).
...
families/{familyId}/transactions/{transactionId}
  ...
  // --- Campos nuevos, opcionales, solo relevantes en modo "shared_expenses" (Fase 18) ---
  paidByUid: string | null        // quién puso el dinero realmente. Ausente/null → se asume igual a `createdBy` (compatibilidad con movimientos ya existentes y con el modo "normal"). Solo se pide explícitamente en el formulario cuando `families/{familyId}.mode == "shared_expenses"` y `type == "expense"`.
  splitAmongUids: array<uid> | null   // miembros entre los que se reparte el gasto. Ausente/null → el gasto NO participa en el cálculo de saldos (comportamiento normal de siempre). Solo se rellena para gastos ("expense") creados en modo de gastos compartidos; los ingresos nunca se reparten (ver más abajo).
  splitMode: "equal" | "custom" | null    // "equal" reparte `amount` a partes iguales entre `splitAmongUids` (redondeo: los céntimos sobrantes se asignan al primer uid de la lista, para que la suma cuadre siempre con `amount`). "custom" usa `splitAmounts`.
  splitAmounts: map<uid, number> | null   // solo si `splitMode == "custom"`; importe exacto que corresponde a cada uid. Debe sumar exactamente `amount` (se valida en cliente antes de guardar); si no cuadra, no se permite guardar el movimiento.

families/{familyId}/settlements/{settlementId}   // (Fase 18) registro de un pago entre dos miembros para saldar (total o parcialmente) su deuda dentro del modo de gastos compartidos. No mueve dinero real ni afecta a `Account.currentBalance` — es solo un apunte informativo de que, fuera de la app, un miembro ya le devolvió dinero a otro.
  fromUid: string                 // quien paga/devuelve el dinero (el que debía)
  toUid: string                   // quien lo recibe (a quien se le debía)
  amount: number                  // siempre positivo
  note: string | null             // texto libre opcional, ej. "Bizum del 3 de agosto"
  createdBy: uid                  // quien registra el saldo en la app (normalmente `fromUid` o `toUid`, pero un admin/owner también puede registrarlo en nombre de otros)
  createdAt: timestamp
```
...
- **⚠️ Hueco detectado (2026-08-02)**: esta sección y `POLITICA_PRIVACIDAD.md` se escribieron antes de la Fase 16 y nunca se actualizaron al añadir anuncios (AdMob) y compras (RevenueCat). Ni la política pública ni el texto de `PrivacyConsentFragment`/`strings.xml` mencionan hoy que se comparten datos técnicos con Google AdMob (identificador de publicidad) ni con RevenueCat (identificador de compra), a pesar de que el código ya los usa. Corregido como tarea explícita en la nueva Fase 17 de `PLAN_DESARROLLO.md`: hay que actualizar `POLITICA_PRIVACIDAD.md` y sincronizar el mismo contenido en el código Java (recursos de `strings.xml` en `values/` y `values-en/` que alimentan `PrivacyConsentFragment`) antes de publicar, siguiendo el mismo patrón de sincronización ya usado en la Fase 10.

### Monetización: anuncios y suscripción Premium (Fase 16)

> Requisito nuevo (2026-07-29). Objetivo explícito del propietario: monetizar con anuncios que **no interrumpan el uso de la app** (se descartan intersticiales, "app open ads" y cualquier formato bloqueante) y ofrecer una suscripción Premium que los elimine. Compatible con la restricción de 2026-07-21 de no vincular ningún medio de facturación a Firebase/Google Cloud.
>
> **⚠️ Actualización 2026-08-02 — la app aún no está publicada en Google Play**: todo lo anterior está implementado a nivel de código con IDs/productos de prueba, pero no verificado contra una ficha real de Play Console. Mientras esto no ocurra, la monetización se considera **incompleta**, no solo pendiente de "pulido". Ver la nueva Fase 17 de `PLAN_DESARROLLO.md` para el checklist de publicación (dividido en tareas delegables a un agente de IA y tareas que exigen obligatoriamente al humano: alta de cuenta de desarrollador, pagos, cuestionario IARC, capturas de pantalla reales).

**Optimización de cómputo para usuarios Premium (2026-08-02)**: hoy el `AdView`/`ViewType` de anuncio se instancia siempre y solo se oculta cuando se resuelve `isPremium == true` de forma asíncrona. Esto es correcto a nivel visual pero desperdicia cómputo/red en usuarios que no van a ver ningún anuncio. Se añade a la Fase 16 la tarea de esperar a conocer `isPremium` **antes** de instanciar el `AdView` o inicializar `AdsRepository`, de modo que un usuario Premium o en Whitelist no dispare ninguna petición relacionada con AdMob en ningún momento de la sesión.

- **Modelo de datos**: se amplía `users/{uid}` con `isPremium` (boolean, `false` por defecto), `premiumProductId`, `premiumPurchaseToken` y `premiumUpdatedAt`.
- **Lista Blanca (Whitelist)**: Nueva colección `premium_whitelist/{email}` que permite otorgar Premium gratuito a desarrolladores y familiares. Si el email del usuario existe aquí, la app activa `isPremium: true` automáticamente.
- **Monetización: anuncios y suscripción Premium** (Fase 16). Objetivo explícito del propietario: monetizar con anuncios que **no interrumpan el uso de la app** (se descartan intersticiales, "app open ads" y cualquier formato bloqueante) y ofrecer una suscripción Premium de por vida (pago único) que los elimine. La seguridad se garantiza mediante una combinación de **RevenueCat** (verificación gratuita en servidor), una **Whitelist** en Firestore (para acceso gratuito de familiares/desarrolladores) y **verificación RSA local** de firmas de Google Play.
- **Diferenciación de Premium en UX**:
    - **Whitelist**: Estrella Verde + texto "Premium activo (Whitelist)".
    - **Compra normal**: Estrella Amarilla + texto "Premium activo".
    - **Cuenta gratuita**: Estrella Gris + texto "Comprar Premium".
- **RevenueCat**: Gestiona el producto de pago único (Non-consumable) y permite restaurar la compra en múltiples dispositivos.
- **Puntos donde se muestran anuncios (solo si `isPremium == false`)**:
  1. Banner adaptativo fijo en la parte inferior del Dashboard y de Estadísticas.
  2. Tarjeta de anuncio banner intercalada cada N movimientos en el listado de `TransactionListFragment`.
- **Consentimiento de anuncios (GDPR/UMP)**: Se usa el SDK de User Messaging Platform de Google.
- **Verificación RSA Local**: Como capa adicional de seguridad "Zero-Server", la app verifica la firma digital del recibo de compra usando la clave pública de Google Play antes de confiar en el estado Premium de forma offline.

### Modo de gastos compartidos: gastos compartidos y ajuste de saldos entre miembros (Fase 18)

> Requisito nuevo (2026-08-10). Objetivo explícito del propietario: que una familia pueda funcionar, opcionalmente, como un sistema de gastos compartidos — gastos que pagan unos miembros pero que se reparten entre varios, con un cálculo de quién debe cuánto a quién y un botón para marcar un saldo como saldado. **La funcionalidad de familia "normal" (posición neta conjunta, sin repartos) se mantiene intacta y sigue siendo el modo por defecto**; el modo de gastos compartidos es un ajuste explícito, no un reemplazo.

- **Activación por familia, no global**: `families/{familyId}.mode` (`"normal"` por defecto). Solo un `admin`/`owner` puede cambiarlo, desde `FamilySettingsFragment` (mismo sitio que editar nombre/moneda), con un diálogo de confirmación que explica el cambio de UX antes de aplicarlo. Cambiar de modo es reversible en cualquier momento y **no borra ni recalcula nada**: los campos de reparto (`paidByUid`, `splitAmongUids`, etc.) se quedan en los movimientos que ya los tuvieran, simplemente dejan de mostrarse/usarse mientras la familia esté en modo "normal".
- **Por qué un campo por familia y no por usuario**: el reparto de gastos y el cálculo de saldos solo tiene sentido si todos los miembros ven los mismos números; permitir que cada miembro eligiera su propia vista rompería la consistencia de los saldos. Es una decisión de la unidad familiar, igual que la moneda.
- **Qué cambia en la UX al activar el modo de gastos compartidos** (sin tocar nada de lo que ya existe en modo normal: Cuentas, Categorías, importación/exportación, Estadísticas siguen funcionando igual):
  - **Formulario de movimiento** (`AddEditTransactionFragment`): para gastos (`type == "expense"`) aparecen dos campos nuevos, solo visibles en modo de gastos compartidos:
    1. **"¿Quién pagó?"** (`paidByUid`): selector de un único miembro de la familia, preseleccionado al usuario actual.
    2. **"¿Entre quién se reparte?"** (`splitAmongUids` + `splitMode`/`splitAmounts`): multi-selección de miembros (preseleccionados todos por defecto) con un interruptor "Partes iguales" (por defecto) / "Importes personalizados". En "Importes personalizados" se muestra un campo editable por cada miembro seleccionado con validación en tiempo real de que la suma coincide exactamente con `amount` antes de permitir guardar.
    - Los ingresos (`type == "income"`) no se reparten nunca: se siguen registrando igual que en modo normal, porque el escenario típico de gastos compartidos es "quién ha pagado de más en gastos comunes", no repartir ingresos.
    - Si un movimiento no lleva `splitAmongUids`, se trata como un gasto puramente personal/familiar normal y no entra en el cálculo de saldos (permite mezclar gastos compartidos y gastos privados dentro de la misma familia).
  - **Nueva pantalla "Saldos"** (`ui/sharedexpenses/BalancesFragment`), accesible desde una nueva pestaña/entrada de navegación visible **solo** cuando `mode == "shared_expenses"`:
    - Lista, por miembro, su saldo neto actual (positivo = "le deben X", negativo = "debe X"), calculado en cliente (ver algoritmo debajo).
    - Debajo, la lista de **saldos simplificados sugeridos** ("Ana debe 23,50 € a Pedro"), cada uno con un botón **"Marcar como saldado"** que abre un diálogo de confirmación (importe editable, por si el pago real fue parcial) y crea un documento en `families/{familyId}/settlements`.
    - Historial de saldos ya registrados (settlements), con posibilidad de deshacer/eliminar un settlement creado por error (mismo criterio de permisos que crearlo, ver reglas de seguridad).
- **Cálculo de saldos (100% en cliente, igual criterio que Estadísticas — sección 4)**: para cada gasto con `splitAmongUids` no vacío:
  - El `paidByUid` (o `createdBy` si `paidByUid` está ausente) "adelantó" `amount`.
  - Cada uid de `splitAmongUids` "debe" su parte (`amount / n` en reparto igual, o `splitAmounts[uid]` en reparto personalizado).
  - Saldo neto de un miembro = (total que ha pagado en gastos compartidos) − (total que le corresponde pagar en gastos compartidos) + (settlements donde `toUid == miembro`) − (settlements donde `fromUid == miembro`).
- **Simplificación de deudas ("quién le paga a quién")**: en vez de mostrar todas las combinaciones cruzadas, se aplica el algoritmo greedy estándar de minimización de transacciones: ordenar miembros por saldo neto (deudores con saldo negativo, acreedores con saldo positivo) y emparejar iterativamente el mayor deudor con el mayor acreedor por el importe mínimo entre ambos, repitiendo hasta que todos los saldos queden en 0. Minimiza el número de pagos sugeridos sin cambiar el resultado neto de nadie.
- **Un settlement nunca toca `Account.currentBalance` ni crea un `Transaction`**: es un apunte puramente informativo entre miembros (coherente con la decisión ya tomada el 2026-07-29 de que los traspasos entre cuentas tampoco generan `Transaction`). Si el propietario quisiera en el futuro que saldar una deuda sí moviera dinero real entre cuentas, es una ampliación a decidir aparte, no incluida en esta fase.
- **Asunciones tomadas al no estar especificado en el enunciado original** (a confirmar con el propietario del producto):
  - Un movimiento con reparto personalizado (`splitMode: "custom"`) exige que la suma de `splitAmounts` sea exactamente igual a `amount`; no se permite guardar un reparto que no cuadre, para que el cálculo de saldos nunca quede descuadrado silenciosamente.
  - Editar o borrar un movimiento que ya tiene reparto recalcula los saldos automáticamente la próxima vez que se abra "Saldos" (el cálculo no se persiste, se deriva siempre de los datos vivos), igual que ya ocurre con las Estadísticas.
  - Cambiar la familia de modo "shared_expenses" a "normal" y de vuelta a "shared_expenses" no reinicia los saldos: como no se borra ningún campo, los saldos se recalculan exactamente igual que antes de cambiar de modo, salvo que se hayan añadido/editado movimientos mientras tanto.
  - Se permite mezclar, dentro de una misma familia en modo de gastos compartidos, movimientos con reparto y movimientos normales sin reparto (gastos puramente personales de un miembro que no afectan a nadie más).
...
- **(Fase 18) `families/{familyId}.mode`**: solo un miembro con `role == "admin"` o `role == "owner"` puede escribirlo (mismo criterio que `currencyCode`).
- **(Fase 18) `families/{familyId}/settlements/{settlementId}`**: cualquier miembro aprobado puede leer (necesita ver el estado de todos los saldos, no solo los suyos, para que la lista simplificada tenga sentido). Para crear o borrar un settlement, `request.auth.uid` debe ser igual a `fromUid` o a `toUid` del documento (uno de los dos implicados en el pago), **o** tener `role` `admin`/`owner` en la familia (para poder corregir un registro hecho por error por otro miembro). Nadie puede editar un settlement ya creado (solo crear uno nuevo o borrar); así el historial de ajustes queda como un log append-only más simple de razonar.
- **(Fase 18) `Transaction.paidByUid`/`splitAmongUids`/`splitMode`/`splitAmounts`**: sujetos a las mismas reglas ya existentes sobre creación/edición de `transactions` (cualquier miembro aprobado puede crear/editar); no se añade ninguna restricción de rol nueva sobre estos campos.
...
18. **Monetización: anuncios y suscripción Premium** (Fase 16): anuncios banner no bloqueantes (banner fijo en Dashboard/Estadísticas + tarjeta intercalada cada N movimientos), nunca intersticiales ni formatos que interrumpan el uso. Premium de por vida (pago único) vía Google Play Billing que los elimina por completo. UX diferenciada: Estrella Verde para Whitelist (familiares/devs), Estrella Amarilla para compra normal, Estrella Gris para free. Un usuario Premium/Whitelist no debe disparar ninguna llamada relacionada con AdMob (optimización de cómputo, 2026-08-02). **Incompleta hasta la publicación real en Google Play (Fase 17)**: usa hoy IDs/productos de prueba, no verificados en producción. Ver sección 4.
19. **Publicación en Google Play Store** (Fase 17, nueva 2026-08-02): la app no se ha publicado nunca en Play Console; solo se completó la preparación (icono, ficha, firma). Checklist de lo que falta, con tareas separadas para agente de IA y para el humano (cuenta de desarrollador, pagos, IARC, capturas reales). Ver sección 4 y `PLAN_DESARROLLO.md`.
20. **Modo de gastos compartidos: gastos compartidos y ajuste de saldos** (Fase 18, nueva 2026-08-10): activable por familia (no global) desde Ajustes de familia, solo `admin`/`owner`, sin perder la funcionalidad de familia "normal" (sigue siendo el modo por defecto). En modo de gastos compartidos, los gastos pueden repartirse entre varios miembros (partes iguales o importes personalizados), la app calcula el saldo neto de cada miembro y sugiere la lista simplificada de pagos ("quién le paga a quién") para dejar todo a cero, con un botón "Marcar como saldado" que registra el ajuste sin mover dinero real entre cuentas. Ver sección 4, "Modo de gastos compartidos: gastos compartidos y ajuste de saldos entre miembros (Fase 18)".
...
- **2026-07-29**: **Nueva Fase 16 — Monetización con anuncios y suscripción Premium**. Petición explícita del propietario: los anuncios no deben bloquear el uso de la app, sino mostrarse como una caja/banner en puntos concretos. Se utiliza **RevenueCat** para la gestión de compras y se añade una **Whitelist** en Firestore para acceso gratuito de familiares/desarrolladores.
- **2026-08-02**: **Refactorización de Monetización**: Cambio de modelo de suscripción a **Pago Único de por vida**. Mejora de la UX con indicadores de estrella de colores (Verde: Whitelist, Amarilla: Comprado, Gris: Free). Inicialización perezosa de BillingClient para evitar errores en entornos sin Play Store y corrección de la inicialización de AdMob (GDPR).
- **2026-08-10**: **Nueva Fase 18 — Modo de gastos compartidos (gastos compartidos y ajuste de saldos entre miembros)**. Petición explícita del propietario: poder marcar una familia como "para gastos compartidos", donde se calcule cuánto debe cada miembro al resto, se puedan añadir gastos indicando entre qué miembros se reparten, y exista un botón para marcar un saldo entre dos miembros como saldado. Requisito explícito: la funcionalidad de familia normal se mantiene intacta; el modo de gastos compartidos es un ajuste por familia (`families/{familyId}.mode`), no un reemplazo global, y cambia la UX (formulario de movimientos y nueva pantalla "Saldos") solo para las familias que lo activan. Decisiones clave, documentadas con detalle en la sección 4: (a) el reparto se guarda en el propio `Transaction` (`paidByUid`, `splitAmongUids`, `splitMode`, `splitAmounts`) en vez de en una colección aparte, para no duplicar el histórico de movimientos; (b) los saldos se calculan 100% en cliente sobre datos ya leídos, igual que Estadísticas, sin persistir un "balance" agregado que pudiera desincronizarse; (c) se usa el algoritmo greedy estándar de simplificación de deudas (emparejar mayor deudor con mayor acreedor) para minimizar el número de pagos sugeridos; (d) marcar un saldo como saldado crea un documento en la nueva colección `families/{familyId}/settlements`, que es un apunte informativo y **no** mueve dinero entre cuentas ni crea un `Transaction`, mismo criterio ya usado el 2026-07-29 para los traspasos directos entre cuentas; (e) los ingresos nunca se reparten, solo los gastos, por ser el caso de uso pedido por el propietario.
- **2026-08-02 (revisión de estado, mismo día)**: Se constata que **la app nunca se ha publicado en Google Play Store** — todo el trabajo de la Fase 10 y la Fase 16 es preparación con datos/IDs de prueba, no una publicación verificada en producción. Se añade la **Fase 17** a `PLAN_DESARROLLO.md` con el checklist de publicación real, separando tareas delegables a un agente de IA (generar el `.aab` de release, rellenar textos ya cerrados de la ficha, preparar la URL pública de la política de privacidad) de tareas que exigen obligatoriamente al humano (alta de cuenta de desarrollador de Google Play con pago de 25 USD, verificación de identidad, cuestionario IARC, capturas de pantalla reales, método de cobro). Se documenta la monetización de la Fase 16 como **incompleta**, no solo pendiente de pulido, mientras no se complete la Fase 17. Se añade además una tarea de optimización: no inicializar ni disparar ninguna llamada de `AdsRepository`/AdMob para usuarios `isPremium == true` (hoy solo se oculta la vista tras resolver el estado, pero el SDK puede seguir cargando en segundo plano), para reducir cómputo/red en cuentas Premium. Por último, se detecta que `POLITICA_PRIVACIDAD.md` y el texto de privacidad dentro de la app (`PrivacyConsentFragment`/`strings.xml`, Fase 9 bis) nunca se actualizaron al añadir AdMob y RevenueCat en la Fase 16: no mencionan en ningún punto la publicidad, el identificador de publicidad, el consentimiento UMP ni el encargado de tratamiento de las compras. Se añade la tarea de actualizar ambos (markdown y recursos Java/`strings.xml`) a la vez, dentro de la Fase 16/17, antes de poder publicar.
