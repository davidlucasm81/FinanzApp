...
- **Modelo de datos**: se amplía `users/{uid}` con `isPremium` (boolean, `false` por defecto), `premiumProductId`, `premiumPurchaseToken` y `premiumUpdatedAt`. No se crea ninguna colección nueva; no hace falta que `families/{familyId}` sepa nada de esto, porque la suscripción es por usuario, no por unidad familiar (dos miembros de la misma familia pueden tener estados Premium distintos).
