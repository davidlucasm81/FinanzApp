...
- **Nuevas tarjetas/gráficos (Fase 16)**: tasa de ahorro del periodo, desglose por método de pago (`Transaction.paymentMethod`, ya existente desde la Fase 6), listado de los mayores movimientos individuales del periodo y desglose de gasto por miembro (`Transaction.createdBy`). Ninguna de las cuatro requiere cambios en el modelo de datos de Firestore ni en las reglas de seguridad: se calculan en cliente sobre datos ya leídos, igual que el resto de Estadísticas.
  - **Pendiente de decisión**: si el desglose de gasto por miembro lo ve cualquier miembro aprobado (mismo criterio que el resto de Estadísticas) o solo `admin`/`owner`, por ser la única métrica de esta lista que expone comportamiento individual y no solo cifras agregadas de la familia. No implementar hasta resolver esta ambigüedad; mientras tanto, se documenta aquí como abierta en vez de asumir un valor.
...
- **Filtro de granularidad temporal (Fase 17)**: selector con las opciones Día, Mes (por defecto), Año, Lustro (5 años), Década (10 años) y Total (todo el histórico sin agrupar). Convive con el filtro de fecha/cuenta ya existente (`cvDateRange`): el rango de fechas sigue acotando qué movimientos entran en el cálculo; la granularidad decide cómo se agrupan en el gráfico de evolución (`MonthlySummary` de la Fase 8 se generaliza a un `PeriodSummary` con `label`, `startDateMillis`, `endDateMillis`, `income`, `expense`) y en qué unidad se compara la variación porcentual — siempre entre el último bucket completo y el inmediatamente anterior **del mismo tamaño que la granularidad activa**, no solo mes vs. mes como hasta ahora. Con granularidad "Total" no existe un bucket anterior con el que comparar, así que las tarjetas de variación se ocultan (mismo patrón que ya existe hoy cuando `variation == null`).
  - **Asunción tomada**: con granularidad "Día" y un rango de fechas sin acotar, el número de barras podría dispararse (potencialmente miles). Se asume que hay que exigir al usuario un rango de fechas acotado al elegir esta granularidad, en vez de intentar dibujar todo el histórico día a día; a confirmar/documentar de forma definitiva cuando se implemente la Fase 17.
...
- **2026-07-26**: **Nuevas métricas de Estadísticas y modificación de la vista: filtro de granularidad temporal y navegación desde el gráfico de barras.** Se añaden las Fases 16 y 17 a `PLAN_DESARROLLO.md` y la sección "Estadísticas: nuevas métricas, granularidad temporal y navegación desde el gráfico (Fases 16 y 17)" al punto 4 de este documento.
  - **Fase 16**: tasa de ahorro, desglose por método de pago, mayores movimientos del periodo y gasto por miembro. Ninguna requiere cambios en el modelo de datos ni en las reglas de seguridad. Queda abierta la decisión de si el desglose por miembro es visible para cualquier miembro aprobado o solo para `admin`/`owner`.
  - **Fase 17**: filtro de granularidad (Día/Mes/Año/Lustro/Década/Total, Mes por defecto) que sustituye a la comparativa interanual prevista inicialmente en la Fase 16; la variación porcentual pasa a compararse siempre contra el bucket anterior del mismo tamaño que la granularidad activa, no solo mes contra mes. Se añade también la navegación desde el gráfico de barras de evolución a Movimientos, filtrado por el periodo pulsado y por tipo (ingreso/gasto) — se generaliza "ese mes" del enunciado original a "el bucket pulsado, coherente con la granularidad activa", como asunción a confirmar con el propietario del producto.
  - Asunción adicional tomada: con granularidad "Día" y un rango de fechas sin acotar, se exigirá al usuario acotar el rango para no intentar dibujar miles de barras; pendiente de confirmación definitiva al implementar la fase.
