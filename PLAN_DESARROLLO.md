...
- [ ] **Desglose por método de pago**: nuevo gráfico donut o barras horizontales agrupando `Transaction.paymentMethod` (campo ya existente desde la Fase 6, sin migración de datos necesaria), mismo patrón de leyenda interactiva con clic → navegar a Movimientos que ya usa la distribución por categorías.
- [ ] **Mayores movimientos del periodo**: listado de los N (p. ej. 5) gastos individuales más altos del rango seleccionado — es un ranking de *movimientos*, no de *categorías*, para no reintroducir lo ya descartado. Cada item navega a editar/ver el movimiento.
- [ ] ~~Comparativa interanual~~ → **Absorbida por la Fase 17**: el selector de granularidad (Día/Mes/Año/Lustro/Década/Total) de la Fase 17 cubre este caso de forma más general — eligiendo granularidad "Año" se obtiene la comparativa interanual sin necesidad de una tarjeta aparte. No implementar como tarjeta independiente.
- [ ] **Gasto por miembro de la familia**: agregación del gasto por `Transaction.createdBy` en el periodo seleccionado — el dato ya es visible movimiento a movimiento en Movimientos, aquí solo se suma; no requiere cambios en las reglas de seguridad de Firestore. Confirmar con el propietario del producto si debe verlo cualquier miembro aprobado (mismo criterio que el resto de Estadísticas) o solo admin/owner, ya que a diferencia del resto de métricas expone directamente el comportamiento de cada persona, no solo cifras agregadas de la familia.
- [ ] **UX**: antes de activar todas las tarjetas a la vez, decidir si se muestran siempre o detrás de un "Ver más" para no repetir la sobrecarga visual que motivó la simplificación del 2026-07-19 documentada en `AGENTS.md`. Documentar la decisión con fecha en `AGENTS.md`, igual que el resto de decisiones de UX del proyecto.
- [ ] Ampliar `StatisticsViewModel` con los nuevos `LiveData` reutilizando `processTransactions()` (ya filtrado por cuentas activas y rango de fechas) en vez de duplicar la lógica de filtrado.
- [ ] Probar con una familia con poco histórico (mostrar "Sin datos" en vez de un gráfico roto, igual que el resto de la pantalla) y con varios años de datos (comprobar que los nuevos gráficos no degradan el rendimiento ya optimizado de la Fase 8).
...
- [ ] Añadir un selector de granularidad (chips o menú) con las opciones: Día, Mes (selección por defecto al entrar en la pantalla), Año, Lustro (5 años), Década (10 años) y Total (todo el histórico sin agrupar). Ubicarlo junto al filtro de fecha/cuenta ya existente (`cvDateRange`).
- [ ] Definir cómo interactúan el selector de granularidad y el filtro de fecha personalizado (`MaterialDatePicker`) ya existente: el rango de fechas sigue acotando qué movimientos entran en el cálculo; la granularidad decide cómo se agrupan en el gráfico de evolución y en qué unidad se compara la variación. Si el rango de fechas elegido por el usuario es más corto que la granularidad seleccionada (p. ej. un rango de 2 meses con granularidad "Década"), mostrar un único bucket con ese rango en vez de un error o un gráfico vacío.
- [ ] Generalizar `MonthlySummary` (Fase 8) a un modelo de periodo agnóstico a la granularidad (p. ej. `PeriodSummary` con `label`, `startDateMillis`, `endDateMillis`, `income`, `expense`), calculado en `StatisticsViewModel` a partir de la granularidad activa en vez de asumir siempre "mes".
- [ ] Recalcular la variación porcentual (`getVariationPercentage()` / `getIncomeVariationPercentage()`) siempre entre el último bucket completo y el inmediatamente anterior, **del mismo tamaño que la granularidad activa** (día vs. día anterior, año vs. año anterior, etc.), no solo mes vs. mes como hasta ahora.
- [ ] Caso "Total": no hay un bucket anterior con el que comparar — ocultar las tarjetas de variación (mismo patrón que ya existe hoy cuando `variation == null`) y mostrar un único bloque agregado en vez del gráfico de barras de evolución.
- [ ] Caso "Día" con rangos largos sin acotar: decidir un límite razonable (p. ej. exigir que el usuario acote el rango de fechas al elegir granularidad "Día", para no intentar dibujar miles de barras) y documentar la decisión tomada en `AGENTS.md`.
- [ ] Persistir la granularidad elegida igual que ya se persiste el filtro de fecha/cuenta actual (mismo ciclo de vida del `ViewModel`), para que no se resetee a "Mes" al rotar la pantalla o al volver de Movimientos.

### Navegación desde el gráfico de barras
- [ ] Añadir `binding.monthlyChart.setOnChartValueSelectedListener(this)` (hoy ese listener solo está activo en `categoryPieChart`) y, en `onValueSelected`, distinguir mediante `Highlight.getDataSetIndex()` si se ha pulsado la barra de "Ingresos" o la de "Gastos".
- [ ] A partir del índice del bucket pulsado (`Highlight.getX()`), obtener su `startDateMillis`/`endDateMillis` (ya disponibles en `PeriodSummary`) y navegar a `TransactionListFragment` reutilizando el mismo patrón que `navigateToTransactions()` (args `preselectedStartDateMillis`/`preselectedEndDateMillis`), añadiendo un nuevo argumento `preselectedTransactionType` (`income`/`expense`).
- [ ] Revisar `TransactionListFragment`/`TransactionListViewModel`: confirmar si ya soportan filtrar por tipo de movimiento (ingreso/gasto); si no, añadir ese filtro reutilizando el mismo patrón que el filtro de categoría/fecha ya implementado.
- [ ] **Asunción a confirmar con el propietario del producto**: el enunciado original habla de navegar "a movimientos de ese mes"; se generaliza aquí a "el periodo (bucket) pulsado", coherente con la granularidad activa (p. ej. pulsar una barra con granularidad "Década" navegaría a Movimientos filtrado por esos 10 años), en vez de limitarlo literalmente al mes. Confirmar que esta generalización es la esperada.
- [ ] Si al confirmar el punto anterior se decide acotar Movimientos a rangos muy amplios (Década/Total), revisar si la paginación ya implementada en Movimientos (Fase 6/6 bis) es suficiente o si conviene forzar al usuario a acotar más el rango antes de navegar.
- [ ] Probar: pulsar la barra de Ingresos de un mes → Movimientos filtrado por ese mes y tipo "Ingreso"; pulsar la de Gastos → filtrado por tipo "Gasto"; repetir con granularidad Año y Década para confirmar que el rango de fechas pasado a Movimientos es correcto en cada caso.

### Reglas de seguridad / índices de Firestore
- [ ] No se necesitan reglas de seguridad nuevas (el cálculo sigue siendo 100% en cliente sobre datos ya leídos), pero revisar en la Fase 11 (Observabilidad) si los rangos de fecha más amplios que ahora permite el filtro (Década, Total) necesitan un índice compuesto adicional en las consultas de `TransactionRepository`.
